    ...
    <table className="w-full border text-xs text-right rtl:text-right text-gray-500 dark:text-gray-400">
      ...
      <tbody>
        {items.map((item,index) => {
          return(
          <tr key={item.id} className={`bg-white border-b dark:bg-gray-800 dark:border-gray-700 ${(headerless && index === 0)? "font-bold" : ""}`}>
            <th scope="row" className={`px-2 py-1 text-left font-medium ${ item.bold ? "font-bold" : ""} text-gray-900 whitespace-nowrap dark:text-white`}>
              {item.label}
            </th>
            {heads.map((head,index) => {
                let value = data.find((entry) => entry[lookup] === head)?.[item.id]
                if (value === null) return <td key={index} className='px-2 py-1'></td>
                if (typeof value ==="string") return <td key={index} className='px-2 py-1'>{value}</td>
                if (typeof value === 'number' && !Number.isNaN(value) && percent) {
                  return <td key={index} className='px-2 py-1'>{`${Math.round(value * 100)}%`}</td>
                }
                return(
              <td key={index} className={`px-2 py-1  ${ item.bold ? "font-bold" : ""}`}>
                ...
                <tr>
                  {columns.map((column,index) => (
                    <th
                      key={column.id}
                      scope="col"
                      className={`px-2 py-1 ${(index===0 ? "text-left pl-5" : "")}`}
                      align="center"
                    >
                      {column.label}
                    </th>
                ...
                {rows.map((row, index) => (
                  ...
                  >
                    {columns.map((column,index1) => (
                      <td key={column.id} className={`px-2 py-1 ${(index1===0 ? "text-left pl-5" : "")}`} align="center">
                        {column.percent
